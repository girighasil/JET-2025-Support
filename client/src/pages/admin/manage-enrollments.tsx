...
export default function ManageEnrollments() {
  ...
  const [deleteConfirmEnrollment, setDeleteConfirmEnrollment] =
    useState<any>(null);
  ...
  const filteredEnrollments = selectedCourse
    ? enrollments.filter(
        (enrollment: any) => enrollment.courseId === selectedCourse.id,
      )
    : enrollments;
  ...
  const deleteEnrollmentMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      // Directly update the query cache for faster UI updates
      queryClient.setQueryData(
        ...
        (oldData: any[] = []) => {
          return oldData.filter(
            (enrollment) =>
              !(
                enrollment.userId === variables.userId &&
                enrollment.courseId === variables.courseId
              ),
          );
        },
      );

      // Also update the specific course query
      if (selectedCourse?.id === variables.courseId) {
        queryClient.setQueryData(
          [`/api/enrollments?courseId=${variables.courseId}`],
          (oldData: any[] = []) => {
            return oldData
              ? oldData.filter(
                  (enrollment) => enrollment.userId !== variables.userId,
                )
              : [];
          },
        );
    ...
    onError: (error: Error) => {
      ...
      toast({
        ...
        description:
          error.message || "There was an error deleting the enrollment.",
        variant: "destructive",
      });
    },
  });
  ...
  const handleBatchEnroll = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          error.message || "There was an error creating the enrollments.",
        variant: "destructive",
      });
    } finally {
        ...
        <Card className="bg-gray-50 border-dashed">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-gray-600 max-w-lg mx-auto mb-6">
              Start enrolling students in courses to help them access learning
          ...
          <DialogFooter className="pt-4">
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={
                  selectedStudentIds.length === 0 || batchEnrollmentLoading
                }
              >
      ...
      >
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete this enrollment? This will remove
            ...
            <div className="flex flex-col gap-4 py-4">
              <div className="p-3 border rounded-md bg-gray-50">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    <p className="font-medium">
                      {deleteConfirmEnrollment.studentName}
                    </p>
                    <p className="text-xs text-gray-500">
                      {deleteConfirmEnrollment.studentEmail}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2 pl-11">
                  ...
                  <p className="text-sm">
                    {deleteConfirmEnrollment.courseName}
                  </p>
                </div>
              </div>
            </div>
