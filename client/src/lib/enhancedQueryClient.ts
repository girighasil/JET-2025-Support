...
// Safely checks if a response might be HTML instead of JSON
...
async function parseApiError(res: Response): Promise<Error> {
  try {
    ...
    // Handle HTML responses (common source of the <!DOCTYPE error)
      ...
      // as it would be too verbose and not helpful
...
// Safe JSON parsing function that won't throw on HTML responses
...
export const getQueryFn: <T>(options: {
  ...
  async ({ queryKey }) => {
    try {
      ...
      // Use safe JSON parsing to prevent HTML parsing errors
